...
import com.osmar.boutiqueos.subscription.StripePrices;
...
/** Checkout de Stripe para quien compra desde la landing (todavia sin cuenta). */
@Service
public class StripeCheckoutCreator {
    ...
    public String createCheckoutUrl(String plan) {
        return createCheckoutUrl(plan, null);
    }

    /**
     * @param plan     basic o pro (cualquier otra cosa: basic)
     * @param interval monthly o annual (cualquier otra cosa: mensual)
     */
    public String createCheckoutUrl(String plan, String interval) {
        if (stripeSecretKey.isBlank()) {
            throw new ResponseStatusException(HttpStatus.SERVICE_UNAVAILABLE, "Stripe is not configured");
        }

        PlanType planType = "PRO".equals(plan == null ? "" : plan.trim().toUpperCase()) ? PlanType.PRO : PlanType.BASIC;
        BillingInterval billingInterval = BillingInterval.parse(interval);
        String resolvedPriceId = stripePrices.priceFor(planType, billingInterval);
        if (resolvedPriceId.isBlank()) {
            throw new ResponseStatusException(HttpStatus.SERVICE_UNAVAILABLE,
                    "Stripe price for " + planType.name() + " " + billingInterval.name() + " is not configured");
        }
        ...
        try {
            ...
            form.add("metadata[plan]=" + encode(planType.name()));
            form.add("metadata[interval]=" + encode(billingInterval.name()));
